...
const {save, member_embed} = require('../../globals.js')
...
module.exports = {
  ...
  async execute(message, args) {
    ...
    let embed = member_embed(character, message.guild)

    await message.channel.send({content: (!character.player_id) ? "```You are creating a character without a player ID.\n" +
    ...
    await message.channel.send(`\`\`\`Created new character: ${character.title}\`\`\``)
  }
}
